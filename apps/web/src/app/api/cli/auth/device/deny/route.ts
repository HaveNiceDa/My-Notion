...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await convex.mutation(cliApi.denyCliDeviceAuthSession, {
    ...
    return success({ denied: true });
  } catch (error) {
    return failure(
      "DEVICE_AUTH_DENY_FAILED",
      error instanceof Error ? error.message : "Failed to deny device authorization",
      500,
    );
  }
}
