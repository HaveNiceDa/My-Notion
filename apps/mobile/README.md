...
- **真实移动端工作区**：文档树、最近文档、回收站、收藏、归档、搜索和基础编辑已经跑通。
- **复用 Web Agent 后端**：移动端已接入 Web `/api/agent/stream`，支持流式输出、checkpoint/resume 和当前文档上下文。
- **客户端工程重点明确**：下一阶段不是补页面，而是补移动编辑器、正文图片上传、弱网恢复、本地缓存和真机体验。
- **敏感能力走服务端代理**：AI 请求和图片上传不在客户端暴露 LLM key、EdgeStore key 或 PAT。
