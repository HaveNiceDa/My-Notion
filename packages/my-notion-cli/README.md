...
- Web 设置页会自动准备一个默认 CLI Token，可隐藏、显示、复制和重置；首次使用时复制一次并执行 `auth login`，CLI 会保存到 `~/.my-notion/config.json`，后续默认复用。
- 当本地 token 过期、撤销或失效时，CLI 会提示回到 Web 设置页复制最新默认 Token 并重新登录。
