...
面向人类用户和 AI Agent 的 My-Notion 命令行工具。`my-notion` 通过浏览器 Device Flow 授权访问 My-Notion Machine API，支持文档创建、读取、搜索、更新、导入、导出、归档。MCP 能力已拆到独立包 `@mynotion/mcp`，CLI 仅保留兼容入口。
...
- **Skills + MCP**：随 npm 包发布 Agent Skills；MCP 推荐使用独立 `my-notion-mcp --transport stdio`。
...
| `mcp` | 兼容入口：转调独立 MCP STDIO server |
...
| `my-notion-mcp` | 需要 MCP tool 而不是直接 shell 命令 | 启动独立 STDIO server，写工具保持 `dryRun: true` |
...
2. Profile 专属环境变量：`MY_NOTION_PROD_API_TOKEN`、`MY_NOTION_LOCAL_API_TOKEN`、`MY_NOTION_LOCAL_API_URL` 等
3. 兼容环境变量：`MY_NOTION_WEB_URL`、`MY_NOTION_API_URL`、`MY_NOTION_API_TOKEN`
...
| Tool | 能力 | 默认安全策略 |
| --- | --- | --- |
| `my_notion_readme` | 返回 MCP 使用说明、工具列表和安全规则 | 只读 |
...
该脚本使用 `@modelcontextprotocol/sdk` 的 `Client + StdioClientTransport` 启动 `my-notion-mcp --transport stdio`，覆盖认证失败、工具发现、readme、dry-run 预览、确认后真实创建、读取、追加、搜索、归档清理和测试 PAT 撤销。
