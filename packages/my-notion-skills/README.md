...
My-Notion Skills 是给 AI Agent 的操作手册，不是业务代码。它们约束 Agent 如何安装、认证和调用 `my-notion` CLI / MCP，目标是用最少用户可见输出完成安全的文档读写。
...
| `my-notion-mcp` | 需要 MCP tool 而不是直接 shell 命令 | 启动 MCP STDIO server，暴露搜索、读取、创建和更新工具 |
