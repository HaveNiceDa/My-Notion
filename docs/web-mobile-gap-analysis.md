...
- Mobile 尚未接入 Web Agent `/api/agent/stream` 的完整 NDJSON 事件、checkpoint/resume 和 tool result 协议。
...
| Mobile Agent Stream | 未完成 | 尚未接入 Web Agent `/api/agent/stream`、NDJSON 事件和 checkpoint/resume |
| Mobile RAG / Knowledge Search | 未完成 | 尚未复用 Web RAG / Agent tool 链路 |
| Mobile Tool Call / Web Search | 未完成 | 缺少 Web 同级工具调用与展示协议 |
| Mobile 离线/弱网体验 | 未完成 | 缺少本地草稿、resume cursor、最近会话缓存和网络恢复策略 |
...
### P0：Mobile Agent Stream 最小接入

目标是把 Mobile AI 从当前兼容型 `/api/chat`、`/api/rag` 链路，升级为复用 Web Agent `/api/agent/stream` 的 AI Native 客户端链路。

建议任务：

- 新增 Mobile `agent-stream` client，携带 Clerk Bearer token 请求 Web `/api/agent/stream`。
- 解析 NDJSON 流式事件，优先支持 `run-start`、`text-delta`、`reasoning-delta`、`checkpoint`、`finish`、`error`。
- 在移动端保存 `runId`、`lastAppliedSeq`、`assistantMessageId`，为后续断线续跑做准备。
- 先以最小 UI 展示流式文本和错误状态，复杂 tool UI 暂时以日志或折叠卡片占位。
- 保留 `/api/chat`、`/api/rag` 兼容层作为降级路径，待 Agent Stream 稳定后再收敛。
...
- 用 reducer 或状态机统一管理 `idle/generating/streaming/failed/resumable/done`。
- 支持停止生成、失败重试、继续生成、生成中禁用关键操作。
- 处理 App 切后台、页面卸载、网络中断时的请求取消与状态落盘。
- 对 tool call、write preview、confirmation required 预留状态结构，避免后续接 Web Agent 工具时重构。
- 统一错误分类：鉴权失败、网络失败、服务端错误、协议解析错误、模型错误。
...
1. 先做 Mobile Agent Stream MVP：接入 `/api/agent/stream`，跑通 `run-start -> text-delta -> checkpoint -> finish/error`。
2. 再做 Mobile AI 状态机：停止生成、失败重试、继续生成、生成中禁用关键操作。
3. 验证并补强 Mobile 正文图片上传和移动端编辑器深水区，这是用户感知最明显的编辑缺口。
4. 引入客户端缓存和弱网恢复：草稿、最近会话、resume cursor、网络恢复后续跑。
5. 优化 Mobile 文档树查询模型，降低复杂文档树下的性能风险。
6. 补齐共享状态与 i18n 约定，降低双端维护成本。
7. 回到 Web Agent 的 Plan/MCP adapter 产品化闭环。
