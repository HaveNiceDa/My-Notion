...
- 手机 Expo Go / Dev Client 还连着旧的 Metro server。
- Metro 或 Expo Go 缓存了旧 bundle。
- 电脑上启动的是另一个目录或另一个终端里的 dev server。
- 使用了已安装的 preview/dev build，而不是当前二维码对应的开发会话。

处理顺序：

1. 停掉所有正在运行的 Expo/Metro 进程。
2. 在仓库根目录运行 `pnpm --filter @notion/mobile start:clear`，或本地 AI 调试时运行 `pnpm --filter @notion/mobile dev:local:clear`。
3. 手机端关闭 Expo Go / Dev Client 后重新打开，再扫新的二维码。
4. 如果仍是旧页面，清理 Expo Go 缓存或卸载重装当前 dev build。
5. 确认终端输出的项目路径是 `apps/mobile`，不要扫到其他项目的 Metro 二维码。
...
```bash
# 终端 1：让 Web 服务监听局域网
pnpm --filter @notion/web exec next dev -H 0.0.0.0 -p 3000
