...
- MCP 从 CLI 内部子命令拆成独立 npm 包 `@mynotion/mcp`。
- CLI/MCP 共享内部 `@mynotion/agent-tools`，避免重复维护 Machine API client、dry-run 逻辑和工具契约。
- 默认线上 `prod` 解析已收口：旧 prod endpoint 不再污染最新发布包。
