...
基于现代前端技术栈构建的个人化 Notion 应用，提供类似 Notion 的文档编辑和管理功能。核心特点包括富文本编辑、主题色切换、国际化支持、用户认证、实时数据同步、AI智能对话、响应式设计等。
...
│   └── lib/         # 工具函数
│       ├── rag.ts              # RAG 核心逻辑
│       ├── customEmbeddings.ts  # 自定义 Embeddings
│       └── simpleVectorStore.ts # 简单向量存储
├── convex/          # Convex 后端
│   ├── _generated/  # 自动生成的代码
│   ├── documents.ts # 文档相关函数
│   ├── schema.ts    # 数据模型定义
...
2. **修复文档拖拽 bug**：解决移动侧边栏文件到编辑区时 id 被带入编辑器的问题
3. **页面切换流畅性提升**：优化路由切换动画，减少页面加载时间
4. **更换文章标题图片时的骨架屏效果**：添加更美观的加载状态，提升用户体验
5. **编辑器性能优化**：优化 BlockNote 编辑器的渲染性能，支持更大文档
6. **响应式设计改进**：进一步优化移动设备上的用户体验
7. **错误处理增强**：添加更友好的错误提示和恢复机制
8. **代码分割优化**：减少首屏加载时间，提升应用启动速度
...
2. **移动端应用**：使用 React Native 开发移动端 Notion 应用
   ...
   - 内嵌到原生应用中
   - 保持与 Web 端的数据同步
3. **国际化**：支持更多语言翻译，如日语、韩语、法语等
4. **文章内部搜索替换功能**：支持在文章内部快速搜索和替换文本
5. **AI 总结文章内容**：利用 AI 技术自动总结文章的主要内容
6. **更多编辑器功能**：添加更多富文本编辑功能，如表格、图表、数学公式等
7. **多人协作功能**：支持多人实时协作编辑文档（计划中）
8. **文档侧边栏评论区**：在文档侧边栏添加评论功能，支持对特定内容进行评论和讨论
9. **@人能力**：支持在编辑器和评论中@提及其他用户
10. **更多主题**：提供更多美观的编辑器主题选择
