...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'addDocumentChunks') {
      ...
      console.log(`[Qdrant API] addDocumentChunks - documentId: ${documentId}, chunks count: ${chunks?.length}`);
      if (chunks && chunks.length > 0) {
        console.log(`[Qdrant API] 第一个 chunk 的向量维度: ${chunks[0].embedding?.length}`);
        console.log(`[Qdrant API] 第一个 chunk 的 chunkIndex: ${chunks[0].chunkIndex}`);
        console.log(`[Qdrant API] 第一个 chunk 的 pageContent 长度: ${chunks[0].pageContent?.length}`);
      }
    }
    ...
    switch (action) {
      ...
      case 'addDocumentChunks':
        const { documentId, chunks } = params;
        const result = await vectorStore.addDocumentChunks(userId, documentId, chunks);
      ...
      case 'needsReembedding':
        ...
        const needsReembed = await vectorStore.needsReembedding(reembedDocId, content);
    ...
    console.error('Qdrant API error details:', error.response?.data || error.data || error.message);
